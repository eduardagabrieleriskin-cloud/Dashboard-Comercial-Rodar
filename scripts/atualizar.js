...
// corte padrão = HOJE (data local da atualização). Mudança pedida pela Talita em 25/09/2026: o painel
// passa a ser atualizado manualmente na hora em que ela envia os relatórios, então entra o dia corrente.
// (Antes era ONTEM — regra da Eduarda de 19/08, porque o dia corrente ainda está incompleto no Siprov.)
// Data local, não toISOString(): em UTC, à noite no Brasil já seria o dia seguinte.
function isoHoje() { const d = new Date(); const p = n => String(n).padStart(2, '0'); return d.getFullYear() + '-' + p(d.getMonth() + 1) + '-' + p(d.getDate()); }
