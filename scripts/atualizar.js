/*
 * Atualiza o Painel Comercial Rodar Mutual a partir da exportação BASE mais
 * recente do Siprov (BASE_*.xlsx) salva na pasta Downloads.
 *
 * Fluxo: acha o BASE mais novo -> transforma (adesões até ONTEM) -> injeta no
 * template -> valida -> commit + push (GitHub e Vercel atualizam via git).
 *
 * NÃO acessa o Siprov. Quem exporta o BASE é a usuária; este script publica.
 */
const fs = require('fs');
const path = require('path');
const { execSync } = require('child_process');
const build = require('./transformador_base');
...
function acharBase() {
  const arqs = fs.readdirSync(DOWNLOADS)
    .filter(f => /^BASE_\d{8}.*\.xlsx$/i.test(f))
...
try {
  const base = acharBase();
  if (!base) { log('nenhum BASE_*.xlsx em Downloads. Nada a fazer.'); process.exit(0); }
  const assinatura = base.f + '|' + Math.round(base.m);
  const marca = fs.existsSync(MARKER) ? fs.readFileSync(MARKER, 'utf8').trim() : '';
  if (marca === assinatura) { log('BASE mais recente (' + base.f + ') já publicado. Nada novo.'); process.exit(0); }

  const ate = isoOntem();
  log('fonte: ' + base.f + ' | adesões até ' + ate);
  const res = build(base.full, ate);
  ...
  let html = fs.readFileSync(TEMPLATE, 'utf8').replace('__DATA__', JSON.stringify(res.data));
  ...
  execSync('git commit -m "auto: painel a partir de ' + base.f + ' (adesoes ate ' + ate + ')"', { cwd: REPO });
